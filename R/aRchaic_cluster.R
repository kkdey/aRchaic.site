...
aRchaic_cluster = function(folders,
  ...
  if(run_from == "gom" | run_from == "start"){
    ...
    if(gom_method == "full"){
      ...
      if(!file.exists(paste0(output_dir, "model.rda"))){
        supressWarnings(topic_clus <- do.call(maptpx::topics, append(list(counts = pooled_data, K=K, tol=tol, model = "independent", signatures = signature_pos), topics.control)))
        save(topic_clus, file = paste0(output_dir, "model.rda"))
      }else{
        topic_clus <- get(load(paste0(output_dir, "model.rda")))
      }
  ...
  if(save_plot){
    ...
    b <- do.call(StructureGGplot, append(list(omega= omega, annotation = annotation, palette = topic_cols, output_dir = output_dir), structure.control))
    print(b)
    dev.off()
  }
  if(!save_plot){
    if(is.null(output_dir)){ output_dir <- paste0(getwd(), "/")}
    do.call(StructureGGplot, list(omega= omega, annotation = annotation, palette = topic_cols), structure.control)
    do.call(damageLogo_five, append(list(theta_pool = topic_clus$theta, output_dir = output_dir, save_plot = save_plot),
            logo.control))
  }
